...
class GlobalExceptionHandler {
        ...
        fun handleMissingParam(
                ...
                val paramName = ex.parameter.name ?: "unknown"
                val body =
                        ApiResponse<Nothing>(
                                statusCode = 400,
                                statusMessage = "Missing required field: $paramName"
                        )
                return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(body)
        }
        ...
        fun handleUnreadable(
                ...
                val body =
                        ApiResponse<Nothing>(statusCode = 400, statusMessage = Messages.BAD_REQUEST)
                return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(body)
        }
        ...
        fun handleDataIntegrity(
                ...
                val rootMsg = (ex.rootCause?.message ?: ex.message ?: "").lowercase()
                val msg =
                        when {
                                rootMsg.contains("users.username") ->
                                        Messages.USERNAME_ALREADY_EXISTS
                                rootMsg.contains("users.email") -> Messages.EMAIL_ALREADY_EXISTS
                                rootMsg.contains("users.mobile") -> Messages.MOBILE_ALREADY_EXISTS
                                rootMsg.contains("duplicate") -> Messages.USER_ALREADY_EXISTS
        ...
        fun handleGeneric(ex: Exception): ResponseEntity<ApiResponse<Nothing>> {
                val body =
                        ApiResponse<Nothing>(
                                statusCode = 500,
                                statusMessage = Messages.INTERNAL_ERROR
                        )
