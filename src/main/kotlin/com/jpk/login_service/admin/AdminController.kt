...
class AdminController(
    private val passwordMigrationService: PasswordMigrationService
) {
    
    /**
     * Endpoint to migrate passwords from old format to new BCrypt format
     * This should be called once after deploying the new password system
     */
        ...
        return ResponseEntity.ok(
            ApiResponse(
                ...
                data = MigrationStatusResponse(
                    ...
                    migrationMessage = if (unmigratedCount > 0) {
    ...
    /**
     * Endpoint to migrate a specific user's password with raw password
     * This should be used when raw password is available for proper migration
     */
    ...
    fun migrateUserPassword(
        ...
        val success = passwordMigrationService.migrateUserPassword(
            request.userId, 
            request.rawPassword
        )
        ...
        return ResponseEntity.ok(
            ApiResponse(
                ...
                statusMessage = if (success) "User password migrated successfully" else "Failed to migrate user password",
                data = UserMigrationResponse(
...
data class MigrateUserPasswordRequest(
    val userId: Long,
