...
/**
 * Service for migrating existing passwords to new double-protection system This should be run once
 * after deploying the new password system
 */
@Service
class PasswordMigrationService(private val userRepository: UserRepository) {

    /**
     * Migrate passwords from old system to new BCrypt system Note: This method requires manual
     * intervention for proper migration because we need the raw passwords to generate proper
     * SHA-256 hashes
     */
    @Transactional
    ...
    /**
     * Migrate a specific user's password (with raw password) This should be used when the raw
